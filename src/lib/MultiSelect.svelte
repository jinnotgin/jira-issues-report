<script>
  ...
  onMount(() => {
    for (const o of slot.querySelectorAll("option")) {
      if (o.value === "") continue;
...
<style>
  ...
  li:not(.selected):hover {
    /* background-color: hsl(214, 17%, 92%); */
    background-color: hsl(194, 17%, 92%);
  }
  li.selected {
    /* background-color: hsl(232, 54%, 41%); */
    background-color: hsl(212, 54%, 41%);
    color: white;
  }
  li.selected:nth-child(even) {
    /* background-color: hsl(232, 50%, 45%); */
    background-color: hsl(212, 50%, 45%);
    color: white;
  }
  li.active {
    /* background-color: hsl(214, 17%, 88%); */
    background-color: hsl(194, 17%, 88%);
  }
  li.selected.active,
  li.selected:hover {
    /*background-color: hsl(232, 48%, 50%);*/
    background-color: hsl(212, 48%, 50%);
  }
...
<div class="multiselect" class:readonly>
  <div class="tokens" class:showOptions on:click={handleTokenClick} on:keypress={handleTokenClick}>
    ...
    <div class="actions">
      {#if !readonly}
        <input id={id} autocomplete="off" bind:value={inputValue} bind:this={input} on:keyup={handleKeyup} on:blur={handleBlur} placeholder={Object.keys(selected).length === 0 ? placeholder : ""}/>
